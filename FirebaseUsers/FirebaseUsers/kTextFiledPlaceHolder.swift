...
//            underLine.frame = CGRect(x: 20, y: self.frame.size.height-1, width : self.frame.size.width, height : 1)
//            
//            underLine.clipsToBounds = true
            ...
            UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 1, options: .curveEaseInOut, animations: {() -> Void in
                if (self.text == nil) || (self.text?.characters.count)! <= 0 {
                    ...
                    if self.directionMaterial == placeholderDirection.placeholderUp {
                        self.lblPlaceHolder.frame = CGRect(x : self.lblPlaceHolder.frame.origin.x, y : -self.difference-5, width : self.frame.size.width, height : self.frame.size.height)
                    }else{
    ...
    func EnableMaterialPlaceHolder(enableMaterialPlaceHolder: Bool){
        ...
        self.lblPlaceHolder.frame = CGRect(x: 70, y : 0, width : 0, height :self.frame.size.height)
...
extension kTextFiledPlaceHolder
{
